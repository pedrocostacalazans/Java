...
public class classeMusica {
  ...
  public static void merge(classeMusica arr[], int l, int m, int r) {
    // Find sizes of two subarrays to be merged
    int n1 = m - l + 1;
    int n2 = r - m;

    /* Create temp arrays */
    classeMusica L[] = new classeMusica[n1];
    classeMusica R[] = new classeMusica[n2];
    ...
    int k = l;
    while (i < n1 && j < n2) {
      if (L[i].getEnergy() <= R[j].getEnergy()) {
        arr[k] = L[i];
        i++;
      } else {
        arr[k] = R[j];
  ...
  // merge()
  public static void sort(classeMusica arr[], int l, int r) {
    if (l < r) {
  ...
  public static void fragmentSort(classeMusica[] subConjMusicas, int tam) {
    ...
    while (i < tam - 1) {
      for (int j = i + 1; j < tam; j++) {
        if (subConjMusicas[i].getEnergy() == subConjMusicas[j].getEnergy()) {
          status = true;
        } else if (status == true) {
